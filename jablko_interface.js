...
// Load and export SQLite User Database
const user_db = new sqlite3.Database(jablko_config.database.path, (err) => {
	if (err) {
		console.log(err);
	} else {
		console.log(`Connected to SQLite3 database at "${jablko_config.database.path}"`);
	}
});
